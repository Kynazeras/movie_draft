...
export function LandingPage() {
  ...
  const row2Movies = movies.slice(
    Math.ceil(movies.length / 3),
    Math.ceil((movies.length * 2) / 3),
  );
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background)] flex flex-col relative overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col justify-center gap-4 opacity-40">
        <PosterRow
          movies={row1Movies.length ? row1Movies : FALLBACK_MOVIES.slice(0, 7)}
          direction="left"
          speed="slow"
        />
        <PosterRow
          movies={row2Movies.length ? row2Movies : FALLBACK_MOVIES.slice(7, 14)}
          direction="right"
          speed="normal"
        />
        <PosterRow
          movies={
            row3Movies.length ? row3Movies : FALLBACK_MOVIES.slice(14, 20)
          }
          direction="left"
          speed="fast"
        />
